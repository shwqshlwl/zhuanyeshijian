        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="所属课程" prop="courseId">
              <el-select v-model="homeworkForm.courseId" placeholder="请选择课程" style="width: 100%" @change="onFormCourseChange">
                <el-option v-for="c in courseOptions" :key="c.id" :label="c.name" :value="c.id" />
              </el-select>
            </el-form-item>
          </el-col>
...
const homeworkForm = reactive({ 
  homeworkTitle: '', courseId: '', classId: '', startTime: '', deadline: '', 
...
const getSubmitPercentage = (row) => {
  ...
  const submit = row.submitCount || 0
...
const fetchList = async () => {
  loading.value = true
  try {
    const url = userStore.isStudent ? '/homeworks/student/my' : '/homeworks'
...
const handleAdd = () => {
  ...
  Object.assign(homeworkForm, { 
    title: '', courseId: '', classId: '', startTime: '', deadline: '', 
    content: '', totalScore: 100, allowLate: false 
  })
  formClassOptions.value = []
